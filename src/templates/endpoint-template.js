      ...
      ${this.usePathInNavBar === 'true'
        ...
        : html`
          <div class='title mono-font regular-font-size' part="section-operation-url" style='display: flex; flex-wrap: wrap; color:var(--fg3)'> 
            ...
            <span style="display: flex; flex-wrap: wrap; text-transform: none;" part="label-operation-path">${path.servers?.[0]?.url || this.selectedServer?.computedUrl}${path.path.startsWith('/') ? path.path.slice(1).split('/').filter(t => t.trim()).map(t => html`<span>/${t}</span>`) : path.path.split('/').filter(t => t.trim()).map(t => html`<span>/${t}</span>`)}</span>
          </div>`
      }
